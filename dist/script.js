...
const toggleHam = hamIcon.addEventListener(`click`, function() {
    ...
    if (hamMenu.getAttribute(`hidden`) === `true`) hamMenu.removeAttribute(`hidden`);
    else hamMenu.setAttribute(`hidden`, true);
...
document.addEventListener(`click`, function(event) {
    if (!hamMenu.contains(event.target) && !hamIcon.contains(event.target) && hamMenu.classList.contains(`ham-open`)) {
        //cross back to ham
        midHam.classList.remove(`fade`);
        topHam.classList.remove(`top-transform`);
        topHam.classList.add(`top-reverse`);
...
const visitCount = function() {
    if (localStorage.visitCount) // If it exists, increment it
...
tabsList.addEventListener(`click`, function(e) {
    ...
    for(let i = 0; i < e.target.attributes.length; i++)if (e.target.attributes[i].name === "aria-selected") {
    ...
    if (clickedTabAttr === `false`) {
        ...
        if (tabId === `tab-1`) underline1.classList.add(`underline-right`);
        if (tabId === `tab-2`) underline2.classList.add(`underline-left`);
    }
});

},{"core-js/modules/esnext.symbol.dispose.js":"b9ez5","core-js/modules/web.immediate.js":"49tUX"}],"b9ez5":[function(require,module,exports) {
"use strict";
...
},{}]},["bcmUh","8ZNvh"], "8ZNvh", "parcelRequiree258")
